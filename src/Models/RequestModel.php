...
class RequestModel
{
    ...
    static function delete($id): void
    {
        try {
            ...
            // echo "<script type='text/javascript'>alert('Tu dato se ha borrado!');</script>";
        } catch (PDOException $ex) {
            echo "Error: " . $ex->getMessage();
        }
    }

    public function dateTime($id, $data): void
    {
        try {
            $database = new MysqlConnection;
            $database->mysql->query("SELECT DATE_FORMAT('2003-10-03',GET_FORMAT(DATE,'EUR')) FROM requests WHERE id_request = $id");
        } catch (PDOException $ex) {
            echo "Error: " . $ex->getMessage();
        }
    }
